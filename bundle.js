...
/***/ function(module, exports, __webpack_require__) {
	...
	var letters = __webpack_require__(2)
	var _ = __webpack_require__(3)

	function lettersByDivision(letters) {
	  return letters.reduce((map, letter) => {
	    if (!map[letter.division]) {
	      map[letter.division] = {count: 0, letters: []}
	    }
	    map[letter.division].letters.push(letter)
	    map[letter.division].count = map[letter.division].letters.length
	    return map
	  }, {})
	}

	var dataByDivision = lettersByDivision(letters)
	Object.keys(dataByDivision).forEach((name) => {
	  var division = dataByDivision[name]
	  console.log(name, division.count)
	})
